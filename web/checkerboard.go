...
func Push(s Stack) {
	...
	stack[index] = s             //入栈
	Add(s.x, s.y, -1)            //给周围的点权值-1
	CountWeightForPoint(index)   //根据权值排序 此点之后能走的点的顺序
	index++
}
...
func Pop() Stack {
	p := stack[index]
	Add(p.x, p.y, 1)         //给周围点权值+1
	index--                  //出栈
	checkBoard[p.x][p.y] = 0 //恢复标记
	return p
}
...
func CountWeightForPoint(index int) {
	...
	sort.Slice(stack[index].nextIndex, func(i, j int) bool { //排序
